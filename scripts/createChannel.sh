...
joinChannel() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
    ...
    if grep -Eq "LedgerID already exists|ledger \\[.*\\] already exists with state \\[ACTIVE\\]" log.txt; then
      infoln "${ORG_LABEL} peer already joined channel '${CHANNEL_NAME}', continuing"
      rc=0
      res=0
      break
    fi
  ...
  cat log.txt
  verifyResult $res "After $MAX_RETRY attempts, ${ORG_LABEL} peer has failed to join channel '$CHANNEL_NAME' "
}
...
if [ "${SKIP_ANCHOR_PEERS}" = "true" ]; then
  warnln "Skipping anchor peer updates for this network setup"
else
